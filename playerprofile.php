        ...
        <div class="profile-grid">
            <div class="profil-header">
                <div class="picture-wrapper">
                    <div class="icon"
                         style="background-image: url('<?= 'Resourcen/Icons/' . $userDAO->getIcon($usericonid)->filename ?>');"></div>
                </div>
                ...
                <div class="options-wrapper">
                    ...
                    <?php elseif ($isLoggedIn): ?>
                        ...
                        <?php if ($isFriend): ?>
                            <a class="profile-btn" id="friend-link"
                               href="php/actions/remove_friend_action.php?user=<?= $profileID ?>"></a>
                        <?php else: ?>
                            <a class="profile-btn" id="add-friend-link"
                               href="php/actions/add_friend_action.php?user=<?= $profileID ?>"></a>
                        <?php endif; ?>
                    <?php endif; ?>
                </div>
            </div>
                        ...
                        <?php foreach ($usergames as $usergame): ?>
                            <?php foreach ($allgames as $game): ?>
                                <?php if ($usergame == $game->gameid): ?>
                                    ...
                                    <div class="wrapper">
                                        <li class="card">
                                            <div class="game-container" id="<?php echo "game" . $game->gameid ?>"
                                                 style="<?php echo $style ?>"
                                                 onclick="showStats('<?php echo "game" . $game->gameid ?>', '<?php echo "stats" . $game->gameid ?>')">
                                                <label class="gamelabel"><?php echo $game->gamename ?></label>
                                            </div>
                                        </li>
                                    </div>
                                <?php if ($listDAO->getStatus($game->gameid, $profileID) != 'active'): ?>
                                    <script>deactivate('<?php echo "game" . $game->gameid ?>')</script><?php endif; ?>
                                <?php endif; ?>
                            <?php endforeach; ?>
                        <?php endforeach; ?>
                    </ul>
                            ...
                            <?php foreach ($allgames as $game): ?>
                                <?php if ($usergame == $game->gameid): ?>
                                    <div class=statswrapper id="<?php echo "stats" . $game->gameid ?>">
                                        ...
                                        <div>
                                            <label class="attribute">ELO: </label><label
                                                    class="value"><?php echo $listDAO->getRank($game->gameid, $profileID) ?></label>
                                        </div>
                                        <?php if (count($listDAO->getRoles($game->gameid, $profileID)) > 0): ?>
                                            <div>
                                                <label class="attribute">Positionen: </label><label
                                                        class="value"><?php echo implode(", ", $listDAO->getRoles($game->gameid, $profileID)) ?></label>
                                            </div>
                                        <?php endif; ?>
                                    </div>
                                <?php endif; ?>
